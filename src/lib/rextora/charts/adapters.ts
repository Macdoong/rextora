 ...
 */
import type { UnifiedMetricsSnapshot, UnifiedTradeResult } from "../metrics/types";
...
export function equityCurveToSeries(equity: number[], name = "Equity"): ChartSeries {
  return {
    id: "equity",
    name,
...
export function drawdownFromEquity(equity: number[]): ChartSeries {
  ...
  return { id: "drawdown", name: "Drawdown %", color: CHART_THEME.drawdown, data };
}

export function tradesToMarkers(
  trades: Array<Pick<BacktestTrade, "side" | "entryBar" | "exitBar" | "entryPrice" | "exitPrice" | "exitReason" | "stopLoss" | "takeProfit">>,
  candleTimes?: number[]
): TradeMarker[] {
  ...
  for (const t of trades) {
    ...
    if (t.stopLoss > 0) {
      markers.push({ time: entryTime, price: t.stopLoss, kind: "stop_loss", label: "SL" });
    }
    if (t.takeProfit > 0) {
      markers.push({ time: entryTime, price: t.takeProfit, kind: "take_profit", label: "TP" });
    }
    markers.push({
      time: exitTime,
      price: t.exitPrice,
...
export function backtestTradeTimeline(trades: BacktestTrade[]): TimelineEvent[] {
  return trades.map((t) => ({
    time: t.exitBar,
...
export function winLossDistribution(trades: Array<{ pnlPct: number }>): DistributionBin[] {
  const wins = trades.filter((t) => t.pnlPct > 0).length;
  const losses = trades.filter((t) => t.pnlPct < 0).length;
  const flats = trades.filter((t) => t.pnlPct === 0).length;
  return [
    { label: "Win", value: wins, tone: "up" },
    { label: "Loss", value: losses, tone: "down" },
    { label: "Flat", value: flats, tone: "neutral" }
  ];
}

export function durationDistribution(trades: Array<{ holdBars?: number }>): DistributionBin[] {
  const buckets = [
...
export function feeHistorySeries(trades: Array<{ feePct: number; exitBar: number }>): ChartSeries {
  let cum = 0;
  return {
    id: "fees",
    name: "Fees %",
...
export function rollingWinRate(trades: Array<{ pnlPct: number }>, window = 10): ChartSeries {
  ...
  return { id: "rolling_wr", name: "Rolling Win %", color: CHART_THEME.up, data };
}

export function rollingProfitFactor(trades: Array<{ pnlPct: number }>, window = 10): ChartSeries {
  const data: ChartPoint[] = [];
  for (let i = 0; i < trades.length; i += 1) {
    const slice = trades.slice(Math.max(0, i - window + 1), i + 1);
    const grossWin = slice.filter((t) => t.pnlPct > 0).reduce((s, t) => s + t.pnlPct, 0);
    const grossLoss = Math.abs(slice.filter((t) => t.pnlPct < 0).reduce((s, t) => s + t.pnlPct, 0));
    data.push({ x: i, y: grossLoss > 0 ? grossWin / grossLoss : grossWin > 0 ? 5 : 0 });
  }
  return { id: "rolling_pf", name: "Rolling PF", color: CHART_THEME.accent, data };
}

export function metricsToEquitySpark(metrics: UnifiedMetricsSnapshot): ChartSeries {
  ...
  return { id: "equity", name: "Equity", color: CHART_THEME.equity, data };
}
...
export function metricsFeeFunding(metrics: UnifiedMetricsSnapshot): { fees: ChartSeries; funding: ChartSeries } {
  let feeCum = 0;
  let fundCum = 0;
  ...
  return {
    ...
    funding: { id: "funding", name: "펀딩비", color: CHART_THEME.funding, data: funding }
  };
}

export function unifiedTradesToTimeline(trades: UnifiedTradeResult[]): TimelineEvent[] {
  return trades.map((t) => ({
...
export function positionLevels(input: {
  ...
  const now = Date.now();
  const markers: TradeMarker[] = [
    { time: now - 1, price: input.entry, kind: input.side === "LONG" ? "entry_long" : "entry_short", label: "진입" },
  ...
  const levels: LevelLine[] = [
    ...
    { price: input.current, color: CHART_THEME.accentAlt, label: "현재가", dashed: true }
  ];
  if (input.stopLoss && input.stopLoss > 0) {
    levels.push({ price: input.stopLoss, color: CHART_THEME.stopLoss, label: "손절" });
  }
  if (input.takeProfit && input.takeProfit > 0) {
    levels.push({ price: input.takeProfit, color: CHART_THEME.takeProfit, label: "익절" });
  }
  if (input.liquidation && input.liquidation > 0) {
    levels.push({ price: input.liquidation, color: CHART_THEME.liquidation, label: "청산가", dashed: true });
  }
  if (input.trailing && input.trailing > 0) {
    levels.push({ price: input.trailing, color: CHART_THEME.trailing, label: "트레일링", dashed: true });
  }
...
export function marketStructureLevels(candles: CandlePoint[]): LevelLine[] {
  ...
  const levels: LevelLine[] = [
    { price: resistance, color: CHART_THEME.resistance, label: "저항", dashed: true },
  ...
  for (let i = slice.length - 4; i >= 2; i -= 1) {
    ...
    if (a.high < c.low) {
      ...
      levels.push({ price: mid, color: CHART_THEME.up, label: "FVG↑", dashed: true });
      break;
    }
    if (a.low > c.high) {
      const mid = (a.low + c.high) / 2;
      levels.push({ price: mid, color: CHART_THEME.down, label: "FVG↓", dashed: true });
      break;
    }
  }
  ...
  for (let i = slice.length - 3; i >= 5; i -= 1) {
    ...
    if (impulse > body * 1.5 && next.close > next.open && c.close < c.open) {
      levels.push({ price: c.high, color: CHART_THEME.entryLong, label: "OB↑", dashed: true });
      levels.push({ price: c.low, color: CHART_THEME.entryLong, label: "OB↑L", dashed: true });
      break;
    }
    if (impulse > body * 1.5 && next.close < next.open && c.close > c.open) {
      levels.push({ price: c.high, color: CHART_THEME.entryShort, label: "OB↓H", dashed: true });
      levels.push({ price: c.low, color: CHART_THEME.entryShort, label: "OB↓", dashed: true });
      break;
    }
  }
...
export function signalTimeline(
  ...
  return candidates.map((c, i) => {
    ...
    const scorePart = c.signalScore != null ? `점수 ${c.signalScore.toFixed(0)}` : "";
    return {
      time: Date.now() - (candidates.length - i) * 60_000,
      label: [parts.join(" · "), scorePart, reason].filter(Boolean).join(" · "),
      tone: c.judgment === "진입 가능" ? "up" : c.judgment === "제외" || c.judgment === "보류" ? "down" : "neutral",
      value: c.signalScore
    };
  });
}
...
export function tradeReplayOverlay(trade: {
  ...
  if (trade.exitPrice != null) {
    base.markers.push({
      ...
      kind: "exit",
      label: trade.netPct != null ? `Exit ${trade.netPct}%` : "Exit"
    });
  }
  return base;
}
...
export function coinMeters(coin: {
  ...
  return [
    { label: uiLabel("Trend"), value: clamp01(50 + coin.change24hPct * 5), tone: coin.change24hPct >= 0 ? "up" : "down" },
    { label: uiLabel("Momentum"), value: clamp01(50 + coin.change24hPct * 4), tone: coin.change24hPct >= 0 ? "up" : "down" },
    { label: uiLabel("Volume"), value: clamp01(Math.log10(Math.max(1, coin.quoteVolume)) * 12), tone: "neutral" },
    { label: uiLabel("Volatility"), value: clamp01(coin.volatility * 12), tone: coin.volatility > 5 ? "warn" : "neutral" },
    { label: uiLabel("Signal"), value: clamp01(coin.aiScore), tone: coin.aiScore >= 70 ? "up" : "neutral" },
    {
      label: uiLabel("Score"),
...
export function strategyScatter(
  rows: Array<{ name: string; totalReturn: number; mdd: number; trades?: number }>
): ScatterPoint[] {
  return rows.map((r) => ({
...
export function riskExposureSeries(risk: UnifiedRiskView): ChartSeries {
  return {
    ...
    data: [
      { x: 0, y: risk.openPositions, label: "Open" },
      { x: 1, y: risk.maxPositions, label: "Max" }
    ]
  };
}
...
export function volumeSeries(candles: CandlePoint[]): ChartSeries {
  return {
    ...
    data: candles.map((c, i) => ({ x: i, y: c.volume ?? 0, label: String(c.time) }))
  };
}
