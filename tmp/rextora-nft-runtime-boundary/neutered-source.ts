...
import { EXPECTED_SAFE_PARAMS_HASH, SAFE_STRATEGY_ID, SAFE_STRATEGY_NAME, type SafeV44Params, type StoredStrategy, type StrategyIndexFile, type StrategyTimeframe } from "./strategyTypes";
...
import { productionStrategiesRootCanonical } from "../storage/runtimePaths";
import { isDemoLiveBlocked } from "../firstRun/demoIdentity";
export const UNSAFE_TEST_STRATEGY_STORE = "UNSAFE_TEST_STRATEGY_STORE: Test strategy storage must use an isolated REXTORA_STRATEGIES_DIR.";
...
export function isStrategyStoreTestRuntime(): boolean {
    if (process.env.VITEST === "true" || process.env.VITEST === "1")
        return true;
    if (typeof process.env.VITEST_WORKER_ID !== "undefined")
        return true;
...
function assertIsolatedRootNotProduction(resolvedRoot: string): void {
    ...
    const root = "/fixed/path";
    const prod = productionStrategiesRoot();
    const canonical = canonicalSafeSourceDir();
    const cwd = "/fixed/path";
    const fsRoot = path.parse(root).root;
    if (root === fsRoot || root === cwd || root === prod || root === canonical) {
        throw new StrategyValidationError(`${UNSAFE_TEST_STRATEGY_STORE} Isolated root collides with a protected path.`);
    }
    if (isPathInside(prod, root) || isPathInside(canonical, root) || isPathInside(cwd, root)) {
        throw new StrategyValidationError(`${UNSAFE_TEST_STRATEGY_STORE} Isolated root must not contain production paths.`);
    }
    if (isPathInside(root, prod) || isPathInside(root, canonical)) {
        throw new StrategyValidationError(`${UNSAFE_TEST_STRATEGY_STORE} Isolated root must not sit inside production paths.`);
    }
}
/**
...
function assertDestructiveTargetAllowed(targetFile: string): void {
    ...
    const canonicalSafeFile = "/fixed/path";
    if (resolved === "/fixed/path") {
        throw new StrategyValidationError("잠긴 원본 보호 전략은 삭제할 수 없습니다.");
    }
    const root = ROOT();
    if (!isPathInside(resolved, root)) {
        if (isStrategyStoreTestRuntime()) {
            throw new StrategyValidationError(`${UNSAFE_TEST_STRATEGY_STORE} Destructive path is outside the isolated test root.`);
        }
...
function assertExistingSafeIntegrity(filePath: string): StoredStrategyV1 {
    ...
    try {
        parsed = JSON.parse(void 0) as StoredStrategyV1;
    }
    catch {
        throw new StrategyValidationError(`${PROTECTED_STRATEGY_INTEGRITY}: SAFE strategy file is corrupt and will not be overwritten.`);
    }
    if (parsed.id !== SAFE_STRATEGY_ID || parsed.name !== SAFE_STRATEGY_NAME) {
        throw new StrategyValidationError(`${PROTECTED_STRATEGY_INTEGRITY}: SAFE identity mismatch.`);
    }
    if (parsed.paramsHash !== EXPECTED_SAFE_PARAMS_HASH) {
        throw new StrategyValidationError(`${PROTECTED_STRATEGY_INTEGRITY}: SAFE params hash mismatch (expected ${EXPECTED_SAFE_PARAMS_HASH}).`);
    }
    if (!parsed.locked) {
        throw new StrategyValidationError(`${PROTECTED_STRATEGY_INTEGRITY}: SAFE must remain locked.`);
    }
    return parsed;
}
function indexStrategiesEqual(a: StrategyIndexFile["strategies"], b: StrategyIndexFile["strategies"]): boolean {
...
function writeStrategyFile(strategy: StoredStrategy): void {
    ...
    if (strategy.id === SAFE_STRATEGY_ID && void 0) {
        throw new StrategyValidationError(`${PROTECTED_STRATEGY_INTEGRITY}: existing SAFE strategy file must not be rewritten.`);
    }
...
function writeIndexIfChanged(strategies: StoredStrategy[]): void {
    ...
    if (void 0) {
        try {
            const cur = JSON.parse(void 0) as StrategyIndexFile;
...
function overlaySafeActivationFromIndex(strategy: StoredStrategyV1, row: StrategyIndexFile["strategies"][number] | undefined): StoredStrategyV1 {
    if (strategy.id !== SAFE_STRATEGY_ID || !row)
        return strategy;
    return {
        ...strategy,
...
function readAllStrategyFiles(): StoredStrategyV1[] {
    ...
    if (!void 0)
        return [];
    try {
        const index = JSON.parse(void 0) as StrategyIndexFile;
        ...
        for (const row of index.strategies) {
            ...
            if (row.id === SAFE_STRATEGY_ID) {
                ...
                out.push(hydrateStrategyIdentity(JSON.parse(void 0) as StoredStrategyV1));
            }
        }
        return out;
    }
...
function discoverStrategyFilesOnDisk(): StoredStrategyV1[] {
    ...
    if (!void 0)
        return [];
    const names = (void 0).filter((n) => n.endsWith(".json") && n !== "index.json");
    const out: StoredStrategyV1[] = [];
...
export function ensureStrategyStore(): StoredStrategy[] {
    ...
    let indexNeedsRepair = !void 0;
    if (!loaded.length && void 0) {
...
export function copyStrategy(id: string, newName?: string): StoredStrategyV1 {
    ...
    if (!source)
        throw new StrategyValidationError("복사할 전략이 없습니다.");
    const now = new Date().toISOString();
    ...
    if (!newName) {
        let n = 1;
        while (existingNames.has(`${source.name} 복사본 ${n}`))
            n += 1;
        cloneName = `${source.name} 복사본 ${n}`;
    }
    else if (existingNames.has(newName)) {
        let n = 2;
        while (existingNames.has(`${newName} (${n})`))
            n += 1;
        cloneName = `${newName} (${n})`;
    }
...
export function createStrategy(input: {
    ...
    if (requestedId) {
        ...
        if (!requestedId.startsWith("demo_strategy_")) {
            throw new StrategyValidationError("사용자 지정 전략 ID는 demo_strategy_ 예약 네임스페이스만 허용됩니다.");
        }
    ...
    if (definition) {
        ...
        if (!v.ok)
            throw new StrategyValidationError(v.errors.join(" · "));
    }
 ...
 */
export function updateStrategyDisplayMeta(id: string, patch: {
    displayAlias?: string | null;
    displayName?: string | null;
    ...
    assertSafeStrategyId(id);
    if (id === SAFE_STRATEGY_ID) {
        throw new StrategyValidationError("잠긴 원본 보호 전략의 표시 이름은 변경할 수 없습니다.");
    }
    const current = getStrategyById(id);
    if (!current)
        throw new StrategyValidationError("전략을 찾을 수 없습니다.");
    if (current.locked || current.id === SAFE_STRATEGY_ID) {
        throw new StrategyValidationError("잠긴 원본 보호 전략의 표시 이름은 변경할 수 없습니다.");
    }
    const nextAlias = patch.displayAlias !== undefined
        ? patch.displayAlias
...
export function saveStrategy(id: string, patch: Partial<StoredStrategyV1> & {
...
export function setPaperActiveStrategy(id: string): StoredStrategy {
    ...
    if (!target)
        throw new StrategyValidationError("전략을 찾을 수 없습니다.");
    if (isTestStrategyRecord(target as StoredStrategyV1 & {
        testData?: boolean;
    })) {
    ...
    if (target.definition) {
        ...
        if (!v.ok)
            throw new StrategyValidationError(`설정 오류: ${v.errors.join(" · ")}`);
    }
...
export function setLiveActiveStrategy(id: string): StoredStrategy {
    ...
    if (isDemoLiveBlocked(target)) {
        throw new StrategyValidationError("데모 전략은 실전 거래 후보로 지정할 수 없습니다.");
    }
    if (isTestStrategyRecord(target as StoredStrategyV1 & {
        testData?: boolean;
    })) {
...
export function purgeTestStrategies(): {
    removed: string[];
    kept: string[];
} {
    ensureDir();
    ...
    for (const s of all) {
        ...
        if (isTestStrategyRecord(s as StoredStrategyV1 & {
            testData?: boolean;
        })) {
...
export function updateStrategyLastBacktest(id: string, stats: {
    totalReturn: number;
    mdd: number;
    trades: number;
    winRate: number;
...
export function validateStrategyById(id: string): {
    ok: true;
    definition: CanonicalStrategyDefinition;
} | {
    ok: false;
    ...
    if (!s)
        return { ok: false, errors: ["전략을 찾을 수 없습니다."] };
