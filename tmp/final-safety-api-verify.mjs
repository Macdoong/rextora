...
async function agent(query, extra = {}) {
...
async function main() {
  ...
  const bodyA = planA.json.plan?.typedCommand?.parameters?.createBody;
  const bodyB = planB.json.plan?.typedCommand?.parameters?.createBody;
  const hashA = planA.json.plan?.typedCommand?.requestHash;
  const hashB = planB.json.plan?.typedCommand?.requestHash;
  ...
  const { executeApprovedCommand } = await import(
    "../src/lib/rextora/agent/commandExecutor.ts"
  );
  const { saveCommand, findByIdempotencyKey } = await import(
    "../src/lib/rextora/agent/commandStore.ts"
  );
  const { markApproved } = await import(
    "../src/lib/rextora/agent/typedCommand.ts"
  );

  const cmdA = planA.json.plan?.typedCommand;
  if (cmdA) {
    const approved = markApproved(cmdA);
    saveCommand(approved);
    const first = await executeApprovedCommand(approved);
    const second = await executeApprovedCommand(approved);
    const found = findByIdempotencyKey(approved.idempotencyKey);
    report.idempotency = {
      firstStatus: first.executionStatus,
      secondStatus: second.executionStatus,
      sameJobId: first.jobId === second.jobId,
      storeJobId: found?.jobId ?? null,
      skippedIdempotent: second.executionStatus === "skipped_idempotent",
    };
  }

  report.safeAfter = safeHash();
  report.verdict =
    report.liveSafety.every(
      (r) =>
        r.safetyBlocked === true &&
        ["start_live", "execute_trade"].includes(r.intentType) &&
        r.actionsLen === 0 &&
        !r.hasTypedCommand &&
        !r.hasExecutionResult,
    ) &&
    report.explanatory.every((r) => r.safetyBlocked !== true) &&
    report.searchAb.hashesDiffer &&
    report.searchAb.timeframesMatchPlan &&
    report.idempotency?.skippedIdempotent === true
      ? "REXTORA FINAL SAFETY AND APPROVAL VERIFIED"
      : "REXTORA FINAL SAFETY AND APPROVAL NOT VERIFIED";

  const out = path.join(process.cwd(), "tmp/operator-fix/final-safety-verification.json");
