...
import { assertHarnessDependencies } from "./providerCanaryProbe.mjs";
import { loadProjectEnv } from "./loadProjectEnv.mjs";
import { detectPrimaryTextLeaks } from "./primaryLeakDetector.mjs";
import { runStructuredProviderCanaryProbe } from "./providerCanaryProbe.mjs";

const ROOT = assertHarnessDependencies(import.meta.url);
const OUT = path.join(ROOT, "tmp", "rextora-deployment-release", "lifecycle-52");
...
const SCENARIOS = (process.env.PHASE3_SCENARIOS || "A,B,C,D,E,F,G,H,I,J,K,L,M")
...
function runtimeEnv(base, dirs, port, provider, extra = {}) {
  // Scenario extras (e.g. F: forced invalid Gemini) must win over the healthy
  // default provider — otherwise AI_AGENT_PROVIDER: provider silently undoes them.
  return {
    ...base,
    NODE_ENV: "production",
    PORT: String(port),
...
async function approve(page) {
  ...
  const responsePromise = page.waitForResponse(
    (res) => {
      if (new URL(res.url()).pathname !== "/api/rextora/agent") return false;
      if (res.request().method() !== "POST") return false;
      const post = res.request().postData() ?? "";
      return post.includes("진행해");
    },
    { timeout: 120_000 },
  );
  ...
  let pendingInBody = false;
  try {
    const parsedPost = JSON.parse(post);
    pendingInBody = Boolean(parsedPost?.pendingProposedAction?.actionId);
    fs.writeFileSync(
      path.join(OUT, "last-approve-request-meta.json"),
      JSON.stringify(
        {
          pendingInBody,
          actionId: parsedPost?.pendingProposedAction?.actionId ?? null,
          query: parsedPost?.query ?? null,
    ...
    fs.writeFileSync(
      path.join(OUT, "last-approve-request-meta.json"),
      JSON.stringify({ pendingInBody: false, parseError: true }, null, 2),
    );
  }
  const raw = await response.text();
  if (!raw.trim() || raw.trimStart().startsWith("<")) {
    throw new Error("approve_non_json_response");
  }
  const json = JSON.parse(raw);
  try {
    const parsedPost = JSON.parse(post);
    const createBody =
      parsedPost?.pendingProposedAction?.parameters?.toolPlan?.find(
        (step) => step?.toolId === "search.create",
      )?.arguments?.createBody ?? null;
    const metaPath = path.join(OUT, "last-approve-request-meta.json");
    const meta = JSON.parse(fs.readFileSync(metaPath, "utf8"));
    meta.conclusionPreview = String(json?.conclusionKo ?? "").slice(0, 160);
    meta.intentType = json?.intentType ?? null;
    meta.mode = json?.conversationRoute?.mode ?? null;
    meta.hasExecutionResult = Boolean(json?.executionResult);
    meta.jobId = json?.executionResult?.jobId ?? null;
    meta.stepSummaries = (json?.executionResult?.steps ?? []).map((step) => ({
      toolId: step?.toolId ?? step?.result?.toolId ?? null,
      ok: step?.ok ?? step?.result?.ok ?? null,
      summary: String(step?.result?.summary ?? step?.errorKo ?? step?.error ?? "").slice(0, 400),
    }));
    meta.createBodyKeys = createBody && typeof createBody === "object" ? Object.keys(createBody) : null;
    meta.createBodyBytes = createBody ? JSON.stringify(createBody).length : 0;
    fs.writeFileSync(metaPath, JSON.stringify(meta, null, 2));
    if (createBody) {
      fs.writeFileSync(
        path.join(OUT, "last-approve-createBody.json"),
...
async function scenarioD(ctx) {
  ...
  return {
    ...
    // Exactly-once: re-approve may return the receipt (skipped_idempotent) but
    // must not create another persisted backtest run.
    repeatedApprovalNoDuplicate:
      finalCount === after &&
      (!repeat?.executionResult ||
        repeat.executionResult.alreadyExecuted === true ||
        repeat.executionResult.executionStatus === "skipped_idempotent"),
...
async function scenarioF(ctx) {
  ...
  const providerStarts = started.filter((event) => event.providerAttempt === 1);
  // F asks two conversational turns under a forced-invalid Gemini. Each turn may
  // attempt the provider once and fall back — that is not a duplicate retry loop.
  // A duplicate loop is the same reasoningRequestId starting providerAttempt 1 twice.
  const attempt1Ids = providerStarts.map((event) => event.reasoningRequestId).filter(Boolean);
  const uniqueAttempt1 = new Set(attempt1Ids);
  return {
    jobId,
    ...
    providerStartedCount: providerStarts.length,
    completedCount: completed.length,
    noDuplicateProviderLoop:
      uniqueAttempt1.size === attempt1Ids.length &&
...
async function scenarioG(ctx) {
  ...
  return {
    ...
    comparisonNatural:
      (/달라|차이|조건|같|비교|변경/.test(compareUi.primaryText) ||
        /달라|차이|조건|같|비교|변경/.test(
          [
            compareResponse?.conclusionKo,
            compareResponse?.explanationKo,
            compareResponse?.interpretationKo,
          ]
            .filter(Boolean)
            .join("\n"),
  ...
  const env = runtimeEnv(
    ...
    scenario === "F"
      ? {
          ...
          GEMINI_API_KEY: "controlled-invalid-provider-key",
          // Prevent env OpenAI from defeating the controlled Gemini failure once
          // credential/settings stores are correctly isolated under REXTORA_DATA_DIR.
          OPENAI_API_KEY: "",
          GOOGLE_API_KEY: "",
        }
      : {},
...
async function selectHealthyProvider(baseEnv) {
  // Isolation/debug shortcut: skip the nested canary server when an explicit
  // healthy provider is already known (scenario F still overrides Gemini itself).
  const pinned = process.env.PHASE3_PROVIDER?.trim();
  if (pinned === "openai" || pinned === "gemini") {
    return {
      provider: pinned,
      canary: null,
      verification: null,
      health: { activeProvider: pinned, checks: [] },
      attempts: [],
    };
  }
  const forcedProvider = null;
...
function expectedIsolatedModelsConsoleError(message) {
  // Isolated lifecycle runtimes often lack persisted provider catalog credentials.
  // Agent/Settings may prefetch model lists and log a 400 — not a product defect.
  return (
    /ai-providers\/models\?provider=(openai|gemini)/i.test(message) &&
    /status of 400/i.test(message)
  );
}
...
function passEntry(entry) {
  ...
  if (entry.consoleErrorCount !== 0) {
    const errors = entry.consoleErrors ?? [];
    const allExpected =
      errors.length > 0 &&
      errors.every((message) => expectedIsolatedModelsConsoleError(String(message)));
    if (!allExpected) return false;
  }
...
async function main() {
  ...
  try {
    ...
    for (const width of VIEWPORTS) {
      ...
      for (const scenario of SCENARIOS) {
        ...
        fs.writeFileSync(path.join(OUT, "lifecycle-progress.json"), JSON.stringify({ buildId: BUILD_ID, providerSelection, matrix }, null, 2));
  ...
  fs.writeFileSync(path.join(OUT, "lifecycle-matrix.json"), JSON.stringify(report, null, 2));
