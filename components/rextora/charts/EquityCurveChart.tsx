...
export function EquityCurveChart({
  ...
  return (
    <ChartShell
      ...
      height={height}
      empty={empty}
      legend={list.map((s) => ({ label: s.name, color: s.color ?? CHART_THEME.equity }))}
    >
...
export function DrawdownChart({
  ...
  return <EquityCurveChart title={title} series={series} height={height} area />;
}

export function BalanceCurveChart(props: { series: ChartSeries; height?: number }) {
  return <EquityCurveChart title="Balance Curve" series={props.series} height={props.height} area />;
}

function LinePlot({ ctx, series, area }: { ctx: ChartRenderContext; series: ChartSeries[]; area: boolean }) {
  ...
  const xScale = createLinearScale([viewStart, viewEnd], [pad.left, pad.left + plotW]);
  ...
  return (
    <g>
      ...
      {visible.map((s) => {
        ...
        const d = s.data.map((p, i) => `${i === 0 ? "M" : "L"}${xScale(p.x)},${yScale(p.y)}`).join(" ");
          ...
          <g key={s.id}>
            ...
            <path
              ...
              onMouseMove={(e) => {
                ...
                const nearest = s.data.reduce((a, b) => (Math.abs(xScale(a.x) - x) < Math.abs(xScale(b.x) - x) ? a : b));
                setTooltip({ x, y: yScale(nearest.y), lines: [s.name, formatAxisNumber(nearest.y)] });
              }}
