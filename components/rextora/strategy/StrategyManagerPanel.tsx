...
export function StrategyManagerPanel() {
  ...
  function updateParam<K extends keyof SafeV44Params>(
    key: K,
    value: SafeV44Params[K],
  ) {
        ...
        {selected?.locked ? (
          ...
          <>
            <Button
              ...
              onClick={() =>
                void act("create", {
                  name: `사용자전략_${Date.now().toString(36)}`,
                })
              }
            >
              새 전략 만들기
            ...
            <Button
              ...
              onClick={() =>
                void act("save", {
                  patch: { name: editName, params: editParams },
                })
              }
            >
              전략 저장
            ...
            <tbody>
              {strategies.map((s) => (
                ...
                >
                  ...
                  <td>
                    {s.lastBacktest
                      ? `${(s.lastBacktest.mdd * 100).toFixed(1)}%`
                      : "-"}
                  </td>
                  <td>{s.lastBacktest?.trades ?? "-"}</td>
                  <td>
                    {s.lastBacktest
                      ? `${(s.lastBacktest.winRate * 100).toFixed(1)}%`
                      : "-"}
                  </td>
                  <td>{s.liveActive ? "지정" : "-"}</td>
                  <td>
                    {s.paperActive ? "모의" : s.liveActive ? "실전" : "-"}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <>
          <Card title="전략 상세" data-testid="strategy-details">
            <div className="grid gap-2 text-sm text-slate-300 md:grid-cols-2">
              ...
              <div>
                트레일링: {selected.params.use_trailing ? "사용" : "미사용"}
              </div>
              <div>
                동적 레버리지:{" "}
                {selected.params.use_dynamic_leverage ? "사용" : "미사용"}
              </div>
              <div>
                비용 방어:{" "}
                {selected.params.cost_guard
                  ? `사용 (${uiLabel("cost_guard_k")}=${selected.params.cost_guard_k})`
                  : "미사용"}
              </div>
              <div>
                {displayParamsHashLabel()}:{" "}
                ...
                <label key={key} className="block text-sm text-slate-300">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      updateParam(
                        key,
                        Number(e.target.value) as SafeV44Params[typeof key],
                      )
                    }
                  />
                </label>
