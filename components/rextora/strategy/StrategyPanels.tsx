            ...
            {strategies.map((strategy) => (
              <tr key={strategy.id} className="border-t border-slate-800">
                ...
                <td className="text-green-300">
                  {strategy.validation.full10m.cagr}%
                </td>
                <td className="text-red-300">
                  {strategy.validation.full10m.maxDrawdown}%
                </td>
                <td>{strategy.validation.full10m.winRate}%</td>
                <td>{strategy.validation.full10m.trades}</td>
                <td>{strategy.validation.full10m.score}</td>
                <td>
                  <Badge tone={strategy.liveEligible ? "success" : "danger"}>
                    {strategy.liveEligible ? "검증 완료" : "실전 차단"}
                  </Badge>
                </td>
              </tr>
            ))}
    ...
    >
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Metric
            label="full_10m 수익"
            value={`${strategy.validation.full10m.totalReturn}%`}
            tone="success"
          />
          <Metric
            label="전체 10개월 최대 낙폭"
            value={`${strategy.validation.full10m.maxDrawdown}%`}
            tone="danger"
          />
          <Metric
            label="recent_3m 수익"
            value={`${strategy.validation.recent3m.totalReturn}%`}
            tone="success"
          />
          <Metric
            label="prev_3m 수익"
            value={`${strategy.validation.prev3m.totalReturn}%`}
          />
          <Metric label="거래 수" value={strategy.validation.full10m.trades} />
          <Metric
            label="과최적화 위험"
            value={strategy.validation.overfittingRisk}
          />
          <Metric
            label="비용 스트레스"
            value={`x1 ${strategy.validation.costStress.cost1x}, x1.5 ${strategy.validation.costStress.cost15x}, x2 ${strategy.validation.costStress.cost2x}`}
          />
          <Metric
            label="Jitter 통과율"
          ...
          <Metric
            label="LIVE 적격"
            value={strategy.liveEligible ? "가능" : "차단"}
            tone={strategy.liveEligible ? "success" : "danger"}
          />
          <Metric
            label="공격형 경고"
            value={
            ...
            tone={strategy.type === "공격형 후보" ? "danger" : "default"}
          />
        </div>
      </div>
  ...
  return (
    <Card title="Random Search 전략 생성">
      ...
      <div className="mt-4 rounded-lg border border-violet-500/30 bg-violet-500/10 p-3 text-sm text-violet-100">
        탐색 시작 버튼은 mock API에만 연결됩니다. 생성 후보는 검증 전 거래에
        사용할 수 없습니다.
      </div>
    </Card>
  );
